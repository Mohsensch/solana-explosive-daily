...
def fetch_moralis_tokens():
    """دریافت توکن‌های سولانا با فیلتر 4 ساعته"""
    try:
        print("📡 دریافت داده از Moralis Filtered Tokens API...")
        
        params = {
            "chain": "solana",
            "timeframe": "fourHours",        # 🎯 فیلتر 4 ساعته
            "order": "price_change_percentage_desc",
            "min_price_change": 3,           # حداقل 3% رشد در 4 ساعت
            "min_volume": 50000,            # حداقل حجم 50K
            "min_liquidity": 30000,         # حداقل نقدینگی 30K
            "limit": 30
        }
        ...
        print(f"✅ {len(tokens)} توکن با روند صعودی 4 ساعته دریافت شد")
        return tokens
...
def fetch_dexscreener_fallback():
    ...
    try:
        ...
        print(f"📊 DexScreener: {len(filtered)} توکن با رشد 1 ساعته")
        return filtered
...
def process_tokens(tokens):
    ...
    for token in tokens:
        try:
            if 'address' in token:
                ...
                price_change_4h = float(token.get('price_change_percentage_4h', 0))
                ...
                price_change_4h = token.get('price_change_1h', 0) * 4
    ...
    final_list.sort(key=lambda x: -x['price_change_4h'])
    return final_list[:6]

def format_message(coins):
    """قالب‌بندی پیام"""
    if not coins:
        return "😴 امروز کوین با روند صعودی 4 ساعته پیدا نشد!\n\nفردا دوباره امتحان کن."
    
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    
    msg = "🚀 **لیست کوین‌های با روند صعودی 4 ساعته** 🚀\n"
    msg += f"📆 {now} UTC\n"
    msg += "━━━━━━━━━━━━━━━━━━━━━━\n\n"
    msg += "🎯 **فیلترها:**\n"
    msg += "   • روند صعودی در تایم‌فریم 4 ساعته\n"
    msg += "   • امتیاز امنیتی ≥ 70 (GoPlus)\n"
    msg += "   • حجم ≥ 50K | نقدینگی ≥ 30K\n\n"
    ...
    for i, coin in enumerate(coins, 1):
        ...
        msg += f"📈 رشد 4h: **+{coin['price_change_4h']:.1f}%**\n"
    ...
    msg += "━━━━━━━━━━━━━━━━━━━━━━\n"
    msg += "✅ **فقط کوین‌های با روند صعودی 4 ساعته**\n"
